...
export const isWall = (grid, x, y) => {
    ...
    if (currentCell.type === 'nothing') {
        ...
        if(grid[y+1][x].type === 'nothing' && grid[y-1][x].type === 'floor') return {result: true, isButton: true};
        if(grid[y-1][x].type === 'floor') return {result: true, isButton: false};
        if(grid[y+1][x].type === 'floor') return {result: true, isButton: false};
        if(grid[y][x-1].type === 'floor') return {result: true, isButton: false};
        if(grid[y][x+1].type === 'floor') return {result: true, isButton: false};
    }
...
export const createRoomsFromSeed = (grid, {x, y, width, height}, range = ROOM_SIZE_RANGE) => {
    ...
    roomValues.forEach(room => {
        if (isValidRoomPlacement(grid, room)) {
            ...
            // place door
            grid = placeCells(grid, {x: room.doorx, y: room.doory}, 'door',() => doorTypes(room.doorType));
