...
export const createEntities = (gameMap, level = 1) => {
  ...
  for (let i = 0; i < _.random(1, 3); i++) {
    const weapon = Object.assign(
      {},
      weaponTypes[_.random(0, weaponTypes.length - 1)]
    );
    weapon.type = "weapon";
    weapons.push(weapon);
  }
...
export const moveEntity = (vector, entity = {}, map, state = {}) => {
  const [x, y] = vector;
  ...
  if (map[deltaY][deltaX].type !== "floor") {
  ...
  if (map[deltaY][deltaX].entity) {
    if (
      map[deltaY][deltaX].entity.type === "enemy" ||
      map[deltaY][deltaX].entity.type === "player"
    ) {
